import { getScoreboard, getNews, getAllTeams, getTeam } from '../services/espnService.js';
...
export const fetchScoreboard = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Error al obtener el scoreboard de NBA', error: error.message });
    }
};
...
export const fetchNews = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Error al obtener las noticias de NBA', error: error.message });
    }
};
...
export const fetchAllTeams = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Error al obtener los equipos de NBA', error: error.message });
    }
};
...
export const fetchTeam = async (req, res) => {
    try {
        ...
        res.status(500).json({ message: `Error al obtener el equipo ${req.params.team}`, error: error.message });
    }
};
