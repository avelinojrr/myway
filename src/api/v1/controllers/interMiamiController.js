...
export const fetchInterMiamiEvents = async (req, res) => {
    try {
        ...
        if (events.length === 0) {
            return res.status(404).json({ message: "No se encontraron eventos del Inter de Miami para sincronizar" });
        }

        res.status(200).json({ events });
    } catch (error) {
        res.status(500).json({ message: "Error al procesar los partidos del Inter de Miami", error: error.message });
    }
};
